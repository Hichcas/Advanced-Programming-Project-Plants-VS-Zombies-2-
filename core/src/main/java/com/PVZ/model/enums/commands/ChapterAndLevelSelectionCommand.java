...
public enum ChapterAndLevelSelectionCommand {
    ...
    ENTER_CHAPTER("^\\s*menu\\s+enter\\s+chapter\\s+-c\\s+(?<chapterName>[A-Za-z0-9_-]+)\\s*$") {
        @Override
        public ChapterAndLevelSelectionInputDTO createDTO(Matcher matcher) {
            return new ChapterAndLevelSelectionInputDTO(this, matcher.group("chapterName"), null, null);
        }
    },
    ...
    ENTER_COLLECTION("^\\s*menu\\s+enter\\s+collection\\s*$|^\\s*enter\\s+menu\\s*$") {
        @Override
        public ChapterAndLevelSelectionInputDTO createDTO(Matcher matcher) {
            return new ChapterAndLevelSelectionInputDTO(this, null, null, null);
        }
    },
